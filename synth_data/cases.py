...
#@njit(cache=True)
def make_data(n_sam,threshold=300,dils=20.0*np.power(10,np.arange(4))):
    ...
    for i in range(n_sam.size):
        ...
        index = np.argmax(ks <= threshold) if np.any(ks <= threshold) else len(ks) - 1
...
class case():
    def __init__(self,mus,sigs,rhos,name,threshold=300,dils=20.0*np.power(10,np.arange(4))):
    ...
    def sample_data(self,size=1000000):
        ...
        if isinstance(self.dil_schedule,np.ndarray):
            return make_data(n_sam,self.threshold,self.dil_schedule)
...
case1 = case(np.array([1000,5000,10000]),
            ...
            np.array([2/5,1/5,2/5]),'case1',threshold=50) 

case2 = case(np.array([1000,5000,15000]),
            np.array([100,300,1000]),
            np.array([1/6,3/6,2/6]),'case2',threshold=50) #keep
...
case0 = case(np.array([8000]),
             np.array([500]),
             np.array([1]),'unimodal',threshold=None,dils=200) #keep

casem1 = case(np.array([4000,8000,14000]),
              np.array([200,1500,1000]),
              np.array([.25,.4,.35]),'multimodal_harder',threshold=None,dils=200) #keep

casem2 = case(np.array([8000,16000,24000]),
              np.array([1000,1000,1000]),
              np.array([.3,.2,.5]),'multimodal_easier',threshold=None,dils=200) #keep
